...
class StripeSubscriptionRefundTest extends DuskTestCase
{
    use Hierarchy, DatabaseMigrations, StripeSetup, Environment;

    public function test_subscribe_refund_with_active_subscription(): void
    {
        ...
        $response = $this->actingAs($userCreated)->json(
            'POST',
            '/api/payments/stripe/subscriptions/'.$subscription->id.'/refunds',
        );

        $response->assertStatus(Response::HTTP_BAD_REQUEST);
        $response->assertJson(
            fn (AssertableJson $json) => $json->where('message', trans('payment.subscription_not_cancelled_for_refund'),
            ),
        );
    }

    public function test_subscribe_refund(): void
    {
        $this->runOnlyIntegrations();
        ...
        $this->assertEquals(SubscriptionStatusEnum::REFUND->value, $subscription->custom_status);
        $response->assertJson(
            fn (AssertableJson $json) => $json->where('message', trans('payment.subscription_refunded'),
            ),
        );
    }
}
